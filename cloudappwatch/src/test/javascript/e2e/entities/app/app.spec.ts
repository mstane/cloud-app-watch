...
describe('App e2e test', () => {
  ...
  it('should create and save Apps', async () => {
    ...
    expect(await appUpdatePage.getCommandLineInput()).to.eq('commandLine', 'Expected CommandLine value to be equals to commandLine');
    ...
    if (await selectedServiceFlag.isSelected()) {
      ...
      expect(await appUpdatePage.getServiceFlagInput().isSelected(), 'Expected serviceFlag not to be selected').to.be.false;
    } else {
      await appUpdatePage.getServiceFlagInput().click();
      expect(await appUpdatePage.getServiceFlagInput().isSelected(), 'Expected serviceFlag to be selected').to.be.true;
    }
